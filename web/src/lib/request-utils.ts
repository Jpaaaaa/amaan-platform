...
export function formatStoreType(storeType: string | null, storeTypeOther: string | null): string | null {
...
export function formatIqd(cents: number): string {
  const amount = cents / 100
  return `${new Intl.NumberFormat('en-US', { maximumFractionDigits: 0 }).format(amount)} IQD`
}

export function deviceHasKpiSnapshot(row: {
  ...
  if (row.kpiUpdatedAtMs == null) return false
  return [
    row.kpiMonthRevenueCents,
    row.kpiMonthGrossProfitCents,
    row.kpiMonthSaleCount,
    row.kpiYearRevenueCents,
    row.kpiYearGrossProfitCents,
    row.kpiYearSaleCount,
  ].some((v) => v != null)
}
